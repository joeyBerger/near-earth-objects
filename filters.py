...
is used bythe `query` method to generate a stream of `CloseApproach`
...
subclassesof `AttributeFilter` - a 1-argument callable (on a `CloseApproach`)
constructed from a comparator (from the `operator` module), a r
eference value, and a class method `get` that subclasses can override to
...
class AttributeFilter:
    ...
    comparing some attribute of a close approach (or its attached NEO) to a
    reference value. It essentially functions as a callable predicate for
    whether a `CloseApproach` object satisfies the encoded criterion.

    It is constructed with a comparator operator and a
    ...
    def __init__(self, op, value):
        ...
        a reference value.
                ...
                value=10` will, when called on an approach,
    ...
    def __repr__(self):
        v = self.value
        n = self.op.__name__
        return f"{self.__class__.__name__}(op=operator.{n},value={v})"
...
def create_filters(
    ...
    and represents a filter that selects close approaches whose
    nominal approach distance is at least that far away from Earth.
