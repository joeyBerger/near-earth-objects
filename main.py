#!/usr/bin/env python3
"""Explore a dataset of near-Earth objects and their close
approaches to Earth.
...
command shell that can repeatedly execute `inspect` and `query`
commands without having to wait to reload the database each time.
...
def date_fromisoformat(date_string):
    """Return a `datetime.date` corresponding to a string in
    YYYY-MM-DD format.

    In Python 3.7 , there is `datetime.date.fromisoformat`,
    but alas - we're supporting Python 3.6 .
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Explore past/future approaches of near-Earth objects."
    )
    ...
    inspect = subparsers.add_parser(
        "inspect", description="Inspect an NEO by primary designation/name."
    )
    ...
    inspect_id.add_argument(
        "-n", "--name", help="The IAU name of the NEO to inspect ('Halley')."
    )

    # Add the `query` subcommand parser.
    query = subparsers.add_parser(
        "query",
        description="Query for approaches that "
    ...
    repl.add_argument(
        ...
        "--aggressive",
        action="store_true",
        help="If specified, kill session whenever project file is modified.",
    )
...
def inspect(database, pdes=None, name=None, verbose=False):
    ...
    :param database: The `NEODatabase` containing data on
    NEOs and their close approaches.
    :param pdes: The primary designation of an NEO for which to search.
    :param name: The name of an NEO for which to search.
    :param verbose: Whether to additionally print all of a matching
    NEO's close approaches.
...
class NEOShell(cmd.Cmd):
    ...
    This is a `cmd.Cmd` shell - a specialized tool for command-based
     REPL sessions.
    ...
    def __init__(
        ...
        Creating this object doesn't start the session - for
        that, use `.cmdloop()`.
        ...
        :param aggressive: Whether to kill the session whenever a
        project file is changed.
        :param kwargs: A dictionary of excess keyword arguments
    ...
    def parse_arg_with(cls, arg, parser):
        """Parse the additional text passed to a command, using a
        given parser.

        If any error is encountered (in lexical parsing or argument
        parsing), print the error to stderr and return None.

        :param arg: The additional text supplied after the command.
        :param parser: An `argparse.ArgumentParser` to parse the arguments.
        :return: A `Namespace` of the arguments (produced by
        `parse_args`) or None.
        """
        ...
        try:
            ...
            # The `parse_args` method doesn't actually surface
            # `ArgumentError`s
    ...
    def do_query(self, arg):
        ...
        `--min-distance`, `--max-distance`, `--min-velocity`,
         `--max-velocity`,
    ...
    def precmd(self, line):
        ...
        c = [f for f in PROJECT_ROOT.glob(
            "*.py") if f.stat().st_mtime > _START]
        if c:
            print(
                "These file(s) have been modified since this session began: "
                f"{', '.join(str(f.relative_to(PROJECT_ROOT)) for f in c)}.",
                file=sys.stderr,
            )
            if not self.aggressive:
                print(
                    "To include changes, exit/restart interactive session.",
                    file=sys.stderr,
                )
            else:
                print(
...
def main():
    ...
    cad = args.cadfile
    database = NEODatabase(load_neos(args.neofile), load_approaches(cad))

    # Run the chosen subcommand.
    if args.cmd == "inspect":
        v = args.verbose
        inspect(database, pdes=args.pdes, name=args.name, verbose=v)
    elif args.cmd == "query":
        query(database, args)
    elif args.cmd == "interactive":
        a = args.aggressive
        NEOShell(
            database, inspect_parser, query_parser, aggressive=a
        ).cmdloop()
